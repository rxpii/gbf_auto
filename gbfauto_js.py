...
def enter_quest(driver, url, summons):
    ...
    while True:
        ...
        stamina_type = get_element_by(driver, driver, By.CLASS_NAME,
                "txt-stamina")
        
        if "AP" in stamina_type.text:
            res = "ap"
            num_pots = 2
        else:
            res = "ep"
        ...
        if (low_res(driver)):
            ...
            fill_res(driver, res, num_pots=num_pots)
            continue
...
def low_res(driver):
    ...
    return current_stam < 0

def fill_res(driver, res, num_pots=1, finish_delay=3):
    ...
    '''
    time.sleep(5)
    click_by(driver, driver, By.CSS_SELECTOR, 'div[class="lis-item se"][data-index="' + str(data_index) + '"]')
...
def join_raid(driver, raid_options, summons):
    ...
    popups_visible = checkfor_popups(driver, popup_type="pop-stamina")

    if len(popups_visible) != 0:
       fill_res(driver, "ep", num_pots=10)
...
def raid_sequence(driver, raid_options, summons, time_hrs=1.0, delay_sequence=5):
    time_s = 1.0 * 60 * 60
    time_start = time.time()
...
def ahalo_sequence(driver, summons, time_hrs=1.0, delay_sequence=5):
    ...
    while time.time() < time_start + time_s:
        enter_quest(driver,
            "http://game.granbluefantasy.jp/#quest/supporter/510011/5",
            summons)
